...
import re # ADICIONADO: Essencial para corrigir o erro do WhatsApp
...
# NOVA FUNÇÃO: Limpa o número para o WhatsApp funcionar
...
def buscar_cliente_por_placa(placa_busca):
    ...
    # Mapeia colunas ignorando maiúsculas/minúsculas
    cols = {c.lower(): c for c in df_c.columns}
    
    if "placa" in cols:
        col_real = cols["placa"]
        df_c[col_real] = df_c[col_real].astype(str).str.replace("-", "").str.strip().str.upper()
        res = df_c[df_c[col_real] == placa_clean]
        
        if not res.empty:
            u = res.iloc[-1]
            # Tenta achar telefone em várias colunas possíveis
            tel = u.get("Telefone", u.get("telefone", u.get("WhatsApp", u.get("Celular", ""))))
            
            return {
                "Cliente": u.get("Cliente", u.get("cliente", "")),
                "Telefone": str(tel),
                "Veiculo": u.get("Veiculo", u.get("Carro", "")),
                "Categoria": u.get("Categoria", u.get("categoria", ""))
            }
    return None

# --- PDF CORRIGIDO (Sem o traço feio) ---
def gerar_pdf_orcamento(dados):
    pdf = FPDF()
    pdf.add_page()
    logo = next((f for f in ["logo.png", "Logo.png", "LOGO.png"] if os.path.exists(f)), None)
    if logo: pdf.image(logo, x=10, y=8, w=35) # Tamanho ajustado
    ...
    # Tabela simplificada
    pdf.set_fill_color(220, 220, 220); pdf.cell(190, 10, txt("Descrição dos Serviços"), 1, 1, 'C', 1)
    pdf.set_font("Arial", size=12)
    for s in str(dados['Servicos']).split(','):
        if s.strip():
            pdf.cell(190, 10, txt(f"• {s.strip()}"), 1, 1, 'L')
            
    pdf.ln(5); pdf.set_font("Arial", "B", 14)
    pdf.cell(140, 10, "TOTAL ESTIMADO", 1, 0, 'R'); pdf.cell(50, 10, txt(f"R$ {dados['Total']:.2f}"), 1, 1, 'C')
    
    pdf.ln(20)
    sig = next((f for f in ["assinatura.png", "Assinatura.png"] if os.path.exists(f)), None)
    if sig: 
        x_c = (210 - 50) / 2
        pdf.image(sig, x=x_c, y=pdf.get_y()-10, w=50)
...
def page_dashboard():
    ...
    if not df_v.empty:
        # Limpeza robusta de dados para evitar zeros
        df_v.columns = [c.strip() for c in df_v.columns] # Remove espaços dos nomes das colunas
        for c in ["Total"]:
            if c in df_v.columns: 
                df_v[c] = pd.to_numeric(df_v[c].astype(str).str.replace('R$', '').str.replace('.', '').str.replace(',', '.'), errors='coerce').fillna(0)
        
        # Tenta converter data flexivelmente
                ...
                if b1.button("CONFIRMAR AGENDAMENTO", use_container_width=True):
                    ...
                    if ok:
                        ...
                        if zap: 
                            ...
                            st.markdown(f'<a href="https://wa.me/55{z_clean}?text=Oi {cli}, JM Detail confirma seu agendamento dia {dt.strftime("%d/%m/%Y")} às {hr}." target="_blank"><button style="background:#25D366;color:white;width:100%;border:none;padding:10px;border-radius:5px">ZAP CONFIRMAÇÃO</button></a>', unsafe_allow_html=True)
                
                if b2.button("📄 GERAR PDF", use_container_width=True):
                    d_p = {"Cliente": cli, "Veiculo": veic, "Placa": p_in, "Data": dt.strftime("%d/%m/%Y"), "Servicos": ", ".join(servs), "Total": total}
        ...
        if df_a.empty: st.info("Vazio.")
            ...
            for i, r in df_a.iterrows():
                ...
                if c2.button("📱", key=f"zap_{i}", use_container_width=True):
                    if r.get("Telefone"): 
                        num_limpo = limpar_numero(r["Telefone"])
                        st.markdown(f'<meta http-equiv="refresh" content="0; url=https://wa.me/55{num_limpo}?text=Olá {r["Cliente"]}, seu {r["Veiculo"]} está pronto!">', unsafe_allow_html=True)
