...
import re  # <--- ADICIONADO PARA CORRIGIR O ERRO DO ZAP
...
# --- NOVA FUNÇÃO PARA CORRIGIR O ERRO DE URL DO WHATSAPP ---
def limpar_numero(numero):
    if not numero: return ""
    # Remove tudo que não for dígito (parenteses, traços, espaços)
...
def page_dashboard():
    ...
    if not df_v.empty:
        ...
        # --- CORREÇÃO DO DASHBOARD ZERADO: FLEXIBILIDADE NA DATA ---
        # dayfirst=True ajuda a entender 10/01 como 10 de Janeiro, não 01 de Outubro
    ...
    if not df_d.empty:
        # --- CORREÇÃO DA DATA NAS DESPESAS TAMBÉM ---
                ...
                if escolhidos:
                    ...
                    if col_btn_ag.button("CONFIRMAR AGENDAMENTO", use_container_width=True):
                        ...
                        if ok: 
                            st.success("Agendado!"); 
                            if zap:
                                # --- AQUI ESTAVA O ERRO DO ZAP: LIMPEZA DE URL ---
            ...
            for i, r in df_a.iterrows():
                ...
                with c_btn1:
                    if st.button(f"✅ Concluir", key=f"ok_{i}", use_container_width=True):
                        ...
                        st.rerun()
                with c_btn2:
                    # --- BOTÃO WHATSAPP CARRO PRONTO (CORRIGIDO TAMBÉM) ---
                    if r.get("Telefone"):
                        zap_limpo_2 = limpar_numero(r.get("Telefone"))
                        ...
                        link_pronto = f"https://wa.me/55{zap_limpo_2}?text={msg_pronto}"
