...
# ==============================================================================
# --- 3. ESTILO CSS (ORIGINAL MANTIDO - VISUAL PREMIUM) ---
# ==============================================================================
st.markdown("""
<style>
    /* 1. FONTE POPPINS */
...
# --- ATENÇÃO: AQUI ESTÁ A ÚNICA MUDANÇA (LÊ PLANILHA OU USA BACKUP) ---
@st.cache_data(ttl=60)
def carregar_catalogo():
    # 1. Tentar ler do Google Sheets (Aba: Catalogo)
    try:
        sheet = conectar_google_sheets()
        if sheet:
            ...
            # Verifica se tem a coluna essencial 'Categoria'
            if not df.empty and "Categoria" in df.columns:
                # Converte tudo que não for Categoria para float
                cols_num = [c for c in df.columns if c != "Categoria"]
                for c in cols_num:
                    df[c] = pd.to_numeric(df[c], errors='coerce').fillna(0.0)
                return df
    except:
        pass # Se der erro, usa o código abaixo como backup

    # 2. Backup (Se a planilha falhar, usa este fixo para não quebrar o site)
    return pd.DataFrame({
...
def gerar_pdf(cliente, carro, placa, data_servico, servicos_com_precos, total):
    ...
    # CORREÇÃO DA LOGO PARA CASE SENSITIVE
    logo_file = None
    if os.path.exists("logo.png"): logo_file = "logo.png"
    elif os.path.exists("Logo.png"): logo_file = "Logo.png"
    
    if logo_file: pdf.image(logo_file, x=55, y=10, w=100); pdf.ln(35)
    else: pdf.ln(20)
...
with c_logo2:
    # CORREÇÃO DA LOGO NA TELA PRINCIPAL
    logo_path = None
...
def page_agendamento():
    ...
    # CARREGA O CATÁLOGO (NUVEM OU LOCAL)
    ...
    with tab_new:
        with st.container(border=True):
            ...
            # Garante que a coluna Categoria existe antes de tentar acessar
            if "Categoria" in df_cat.columns:
                ...
                st.error("Erro no Catálogo: Coluna 'Categoria' não encontrada.")
