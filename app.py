...
def obter_icone_e_classe(cat):
    ...
    elif "suv" in c or "picape" in c or "util" in c: return '<i class="bi bi-truck-front-fill"></i>', 'b-suv'
        ...
        try:
            ...
            if df_a.empty: st.info("Vazio.")
                ...
                for i, r in df_a.iterrows():
                    ...
                    # --- CARD COM BADGE VISUAL ---
                    html_card = f"""
                    <div class="agenda-card">
                        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px">
                            <span class="badge-cat {classe_badge}">{r.get("Categoria", "Geral")}</span>
                            <b style="color:#39FF14">{formatar_moeda(val_total)}</b>
                        </div>
                        <div style="font-size:18px; margin-bottom:5px">
                            <b>{icone} {r["Veiculo"]}</b>
                        </div>
                        <div style="color:#ccc; font-size:14px; margin-bottom:5px">{r["Cliente"]} ({r["Placa"]})</div>
                        <div style="display:flex; justify-content:space-between; color:#888; font-size:12px; border-top:1px solid #444; padding-top:5px; margin-top:5px">
                             <span><i class="bi bi-clock"></i> {r["Data"]} - {r["Hora"]}</span>
                        </div>
                        <div style="margin-top:5px; color:#aaa; font-style:italic; font-size:13px">🔧 {r["Servicos"]}</div>
                    </div>
                    """
    ...
    try:
        ...
        if not df.empty:
            ...
            with st.expander("Ver Clientes para Recontato", expanded=False):
                ...
                # Filtra apenas datas válidas para evitar erro
